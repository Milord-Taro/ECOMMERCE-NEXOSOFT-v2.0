namespace ECOMMERCE_NEXOSOFT.Helpers
{
    public static class FormFieldCss
    {
        public const string Name50 = "form-control js-name-50";
        public const string Id10 = "form-control js-id-10";
        public const string Phone10 = "form-control js-phone-10";
        public const string Email100 = "form-control js-email-100";
        public const string Address150 = "form-control js-address-150";
        public const string Store30 = "form-control js-store-30";
        public const string Text100 = "form-control js-text-100";
        public const string Text150 = "form-control js-text-150";
        public const string Text40 = "form-control js-text-40";
        public const string Barcode13 = "form-control js-barcode-13";
        public const string Sku30 = "form-control js-sku-30";
        public const string Nit10 = "form-control js-id-10";
        public const string Password100 = "form-control js-password-100";
    }
}